...
func (cmd *ScaffoldDataSourceCommand) Help() string {
	...
	strBuilder.WriteString("\nUsage: terraform-plugin-codegen-framework scaffold data-source [<args>]\n\n")
	cmd.Flags().VisitAll(func(f *flag.Flag) {
		if f.DefValue != "" {
...
func (cmd *ScaffoldDataSourceCommand) runInternal(ctx context.Context) error {
	fmt.Println("scaffold data source")
