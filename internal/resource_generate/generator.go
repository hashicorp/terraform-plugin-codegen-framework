...
func (d ResourcesModelsGenerator) Process(schemas map[string]GeneratorResourceSchema) (map[string][]byte, error) {
	...
	for name, schema := range schemas {
		...
		m, err := generatorResourceSchema.Model(name)
		if err != nil {
			return nil, err
		}
...
func (g GeneratorResourceSchema) Model(name string) ([]byte, error) {
	var buf bytes.Buffer
	...
	for _, attributeName := range attributeNames {
		...
		case GeneratorListNestedAttribute:
			generatorDataSourceSchema := GeneratorResourceSchema{
...
func (g GeneratorResourceSchema) ModelFields() (string, error) {
	var s strings.Builder
