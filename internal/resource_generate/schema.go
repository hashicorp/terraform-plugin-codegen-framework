...
func (g GeneratorResourceSchema) ImportsString() (string, error) {
...
func (g GeneratorResourceSchema) SchemaBytes(name, packageName string) ([]byte, error) {
...
func (g GeneratorResourceSchema) Models(name string) ([]model.Model, error) {
	...
	for _, attributeName := range attributeNames {
		if nestedAttribute, ok := g.Attributes[attributeName].(Attributes); ok {
			...
			nestedModels, err := generatorResourceSchema.Models(attributeName)
			if err != nil {
				return nil, err
			}
	...
	for _, blockName := range blockNames {
		if nestedBlock, ok := g.Blocks[blockName].(Blocks); ok {
			...
			nestedModels, err := generatorResourceSchema.Models(blockName)
			if err != nil {
				return nil, err
			}
...
func (g GeneratorResourceSchema) ModelsObjectHelpersBytes() ([]byte, error) {
	var buf bytes.Buffer
...
func (g GeneratorResourceSchema) ModelObjectHelpersTemplate(name string) ([]byte, error) {
