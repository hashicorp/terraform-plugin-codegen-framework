// Copyright (c) HashiCorp, Inc.
// SPDX-License-Identifier: MPL-2.0

package resource_generate

import (
	"bytes"
)

// TODO: Field(s) could be added to handle end-user supplying their own templates to allow overriding.
type GeneratorResourceSchemas struct {
	schemas map[string]GeneratorResourceSchema
}

func NewGeneratorResourceSchemas(schemas map[string]GeneratorResourceSchema) GeneratorResourceSchemas {
	return GeneratorResourceSchemas{
		schemas: schemas,
	}
}

func (g GeneratorResourceSchemas) SchemasBytes(packageName string) (map[string][]byte, error) {
	schemasBytes := make(map[string][]byte, len(g.schemas))

	for k, s := range g.schemas {

		b, err := s.SchemaBytes(k, packageName)

		if err != nil {
			return nil, err
		}

		schemasBytes[k] = b
	}

	return schemasBytes, nil
}

func (g GeneratorResourceSchemas) ModelsBytes() (map[string][]byte, error) {
	modelsBytes := make(map[string][]byte, len(g.schemas))

	for name, schema := range g.schemas {
		var buf bytes.Buffer

		generatorResourceSchema := GeneratorResourceSchema{
			Attributes: schema.Attributes,
			Blocks:     schema.Blocks,
		}

		models, err := generatorResourceSchema.Models(name)
		if err != nil {
			return nil, err
		}

		for _, m := range models {
			buf.WriteString("\n" + m.String() + "\n")
		}

		modelsBytes[name] = buf.Bytes()
	}

	return modelsBytes, nil
}

func (g GeneratorResourceSchemas) ModelsObjectHelpersBytes() (map[string][]byte, error) {
	modelsObjectHelpersBytes := make(map[string][]byte, len(g.schemas))

	for name, s := range g.schemas {
		b, err := s.ModelsObjectHelpersBytes()
		if err != nil {
			return nil, err
		}

		modelsObjectHelpersBytes[name] = b
	}

	return modelsObjectHelpersBytes, nil
}
