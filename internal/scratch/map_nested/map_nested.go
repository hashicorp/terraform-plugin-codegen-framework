...
// IR for resource with map nested attribute using associated_external_type
...
// Schema from IR for resource with map nested attribute using associated_external_type

//func (e *thingResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
//	resp.Schema = schema.Schema{
//		Attributes: map[string]schema.Attribute{
//			"configuration": schema.MapNestedAttribute{
...
// Model from IR for data source with map nested attribute using associated_external_type
...
//		Configuration map[string]*ThingConfiguration
//		Name          *string
//	}
//
...
func ThingResourceModelFromCreateThingResponse(ctx context.Context, apiObject apisdkCreateThingResponse) (ThingResourceModel, diag.Diagnostics) {
	...
	thingConfigurationModel, thingConfigurationModelDiags := ThingConfigurationMapFromThingConfiguration(ctx, apiObject.Configuration)
...
// TODO: Should this re-use ThingConfigurationModel.objectAttributeTypes?

func ThingConfigurationMapFromThingConfiguration(ctx context.Context, apiObject map[string]*apisdkThingConfiguration) (types.Map, diag.Diagnostics) {
	objs := make(map[string]types.Object, len(apiObject))
	var diags diag.Diagnostics

	for k, v := range apiObject {
		obj, d := ThingConfigurationObjectFromThingConfiguration(ctx, v)
	...
	mapVal, d := types.MapValueFrom(ctx, types.ObjectType{
...
func (m ThingResourceModel) ToCreateThingRequest(ctx context.Context) (apisdkCreateThingRequest, diag.Diagnostics) {
	...
	configModels := make(map[string]ThingConfigurationModel, len(m.Configuration.Elements()))

	var objs map[string]types.Object
	m.Configuration.ElementsAs(ctx, &objs, false)
	...
	for k, v := range objs {
		...
		configModels[k] = tfThingConfigurationModel
	}

	// Delegate external type conversion to model
	apiThingConfigurations := make(map[string]*apisdkThingConfiguration, len(configModels))
	...
	for k, v := range configModels {
		...
		apiThingConfigurations[k] = apiThingConfiguration
	}
